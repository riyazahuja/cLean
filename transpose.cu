...
__global__ void transpose(int N, float alpha, float beta, float* input, float* output) {
  ...
  __syncthreads();
  float transIdx = (N * i);
  float tileVal = tile[transIdx];
  output[i] = tileVal;
}

int main() {
  printf("CUDA Kernel: transpose\n");
  ...
  transpose<<<gridDim, blockDim, 1024>>>(N, 2.5f, 1.0f, d_input, d_output);
